...
int compress_block_lzma(u64 origsize, u64 *compsize, u8 *origbuf, u8 *compbuf, u64 compbufsize, int level)
{
    ...
    {   switch (res)
        {
            case LZMA_MEM_ERROR:
                errprintf(tr("lzma_easy_encoder(%d): LZMA compression failed "
                    "with an out of memory error.\nYou should use a lower "
                    "compression level to reduce the memory requirement.\n"), level);
                lzma_end(&lzma);
                return FSAERR_ENOMEM;
            default:
                errprintf(tr("lzma_easy_encoder(%d) failed with res=%d\n"), level, res);
                lzma_end(&lzma);
    ...
    {   errprintf(tr("lzma_code(LZMA_RUN) failed with res=%d\n"), res);
        lzma_end(&lzma);
        return FSAERR_UNKNOWN;
    }
    
    if ((res=lzma_code(&lzma, LZMA_FINISH))!=LZMA_STREAM_END && res!=LZMA_OK)
    {   errprintf(tr("lzma_code(LZMA_FINISH) failed with res=%d\n"), res);
        lzma_end(&lzma);
        return FSAERR_UNKNOWN;
    }
...
int uncompress_block_lzma(u64 compsize, u64 *origsize, u8 *origbuf, u64 origbufsize, u8 *compbuf)
{
    ...
    {   errprintf(tr("lzma_auto_decoder() failed with res=%d\n"), res);
        lzma_end(&lzma);
        return FSAERR_UNKNOWN;
    }
    ...
    {   
        if ((res=lzma_code(&lzma, LZMA_RUN)) != LZMA_STREAM_END) // if error
        {
            ...
            {   memlimit+=64*1024*1024;
                ...
                msgprintf(MSG_VERB2, tr("lzma_memlimit_set(%lld)\n"), (long long)memlimit);
            }
            else // another error
            {   errprintf(tr("lzma_code(LZMA_RUN) failed with res=%d\n"), res);
                lzma_end(&lzma);
