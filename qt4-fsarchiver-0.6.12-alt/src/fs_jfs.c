...
int jfs_mkfs(cdico *d, char *partition)
{
    ...
    {   errprintf(tr("mkfs.jfs not found. please install jfsutils on your system or check the PATH.\n"));
        return -1;
    }
...
int jfs_getinfo(cdico *d, char *devname)
{
    ...
    {   ret=-2;
        errprintf(tr("cannot lseek(fd, JFS_SUPER1_OFF, SEEK_SET) on %s\n"), devname);
        goto jfs_getinfo_close;
    }
    
    if (read(fd, &sb, sizeof(sb))!=sizeof(sb))
    {   ret=-3;
        errprintf(tr("cannot read the jfs superblock on [%s]\n"), devname);
        goto jfs_getinfo_close;
    }
    ...
    msgprintf(MSG_DEBUG1, tr("jfs_magic=[%s]\n"), sb.s_magic);
    
    // ---- label
    msgprintf(MSG_DEBUG1, tr("jfs_label=[%s]\n"), sb.s_label);
    ...
    msgprintf(MSG_DEBUG1, tr("jfs_uuid=[%s]\n"), uuid);
