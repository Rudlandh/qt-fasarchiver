...
int dico_add_generic(cdico *d, u8 section, u16 key, const void *data, u16 size, u8 type)
{
    ...
    {   
        ...
        {   last=item;
            ...
            {   errprintf(tr("dico_add_generic(): item with key=%ld is already in dico\n"), (long)item->key);
                return -3;
            }
        }
        last->next=lnew;
...
int dico_get_generic(cdico *d, u8 section, u16 key, void *data, u16 maxsize, u16 *size)
{
    ...
    if (d->head==NULL)
    {   msgprintf(MSG_DEBUG1, tr("dico is empty\n"));
        return -1;
    }
    
    if (maxsize<1)
    {   msgprintf(MSG_DEBUG1, tr("case1: maxsize=%d\n"), maxsize);
        return -3;
    }
    ...
    for (item=d->head; item!=NULL; item=item->next)
    {
        if ((item!=NULL) && (item->key==key && item->section==section))
        {
            ...
            {   msgprintf(MSG_DEBUG1, tr("case2: (item->size > maxsize): item->size =%d, maxsize=%d\n"), item->size, maxsize);
                return -4;
            }
    ...
    msgprintf(MSG_DEBUG1, tr("case3: not found\n"));
    return -5; // not found
}
...
int dico_show(cdico *d, u8 section, char *debugtxt)
{
    ...
    if (d->head)
    {
        for (item=d->head; item!=NULL; item=item->next)
        {
            if (item->section==section)
            {
                snprintf(buffer, sizeof(buffer), tr("key=[%ld], sizeof(data)=[%d], "), (long)item->key, (int)item->size);
                
                switch (item->type)
                {
                    case DICTYPE_U8:
                        snprintf(text, sizeof(text), tr("type=u8, size=[%d]"), (int)item->size);
                        break;
                    case DICTYPE_U16:
                        snprintf(text, sizeof(text), tr("type=u16, size=[%d]"), (int)item->size);
                        break;
                    case DICTYPE_U32:
                        snprintf(text, sizeof(text), tr("type=u32, size=[%d]"), (int)item->size);
                        break;
                    case DICTYPE_U64:
                        snprintf(text, sizeof(text), tr("type=u64, size=[%d]"), (int)item->size);
                        break;
                    case DICTYPE_STRING:
                        snprintf(text, sizeof(text), tr("type=str, size=[%d], data=[%s]"), (int)item->size, (char*)item->data);
                        break;
                    case DICTYPE_DATA:
                        snprintf(text, sizeof(text), tr("type=dat, size=[%d]"), (int)item->size);
                        break;
                    default:
                        snprintf(text, sizeof(text), tr("type=unknown"));
                        break;
                }
    ...
    {
        msgprintf(MSG_FORCE, tr("dico is empty\n"));
    }
    
    msgprintf(MSG_FORCE, tr("-----------------debug-dico-end(%s)------------------\n\n"), debugtxt);
    return 0;
}
