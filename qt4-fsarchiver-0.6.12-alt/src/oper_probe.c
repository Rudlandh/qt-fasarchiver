...
int oper_probe(bool details)
{
    ...
    {   msgprintf(MSG_FORCE, tr("Failed to detect disks and filesystems\n"));
        return -1;
    }
    ...
    if (diskcount>0)
    {
        msgprintf(MSG_FORCE, tr("[======DISK======] [=============NAME==============] [====SIZE====] [MAJ] [MIN]\n"));
        for (i=0; i < res; i++)
    ...
    {
        msgprintf(MSG_FORCE, tr("No physical disk found\n"));
    }
    ...
    {
        msgprintf(MSG_FORCE, tr("No filesystem found\n"));
    }
