...
int extfs_mkfs(cdico *d, char *partition, int extfstype)
{
    ...
    {   errprintf(tr("%s not found. please install a recent e2fsprogs on your system or check the PATH.\n"), progname);
        ret=-1;
    ...
    {   errprintf(tr("this filesystem has ext{2,3,4} features which are not supported by this fsarchiver version.\n"));
        return -1;
    }
    ...
    msgprintf(MSG_VERB2, tr("the filesystem type determined by the original filesystem features is [%s]\n"), format_fstype(origextfstype));
    ...
    msgprintf(MSG_VERB2, tr("the filesystem type to create considering the command options is [%s]\n"), format_fstype(extfstype));
    ...
    for (i=0; mkfeatures[i].name; i++)
    {
        if (mkfeatures[i].firste2p<=e2fstoolsver) // don't pass an option to a program that does not support it
        {
            ...
            {   msgprintf(MSG_VERB2, tr("--> feature [%s]=YES\n"), mkfeatures[i].name);
                strlist_add(&strfeatures, mkfeatures[i].name);
            }
            else
            {   msgprintf(MSG_VERB2, tr("--> feature [%s]=NO\n"), mkfeatures[i].name);
    ...
    {   strlist_merge(&strfeatures, temp, sizeof(temp), ',');
        ...
        msgprintf(MSG_VERB2, tr("features: mkfs_options+=[-O %s]\n"), temp);
    }
    
    // ---- check mke2fs version requirement
    msgprintf(MSG_VERB2, tr("mke2fs version detected: %s\n"), format_prog_version(e2fstoolsver, temp, sizeof(temp)));
    msgprintf(MSG_VERB2, tr("mke2fs version required: %s\n"), format_prog_version(e2fsprogs_minver[extfstype], temp, sizeof(temp)));
    if (e2fstoolsver < e2fsprogs_minver[extfstype])
    {   errprintf(tr("mke2fs was found but is too old, please upgrade to a version %s or more recent.\n"), 
            format_prog_version(e2fsprogs_minver[extfstype], temp, sizeof(temp)));
        ret=-1;
        goto extfs_mkfs_cleanup;
    }
...
int extfs_getinfo(cdico *d, char *devname)
{
    ...
    {   errprintf(tr("ext2fs_open(%s) failed\n"), devname);
        return -1;
    }
    ...
    msgprintf(MSG_DEBUG1, tr("extfs_uuid=[%s]\n"), uuid); 
    ...
    msgprintf(MSG_DEBUG1, tr("extfs_max_mount_count: %ld\n"), (long)max(super->s_max_mnt_count,0));
    msgprintf(MSG_DEBUG1, tr("extfs_check_interval: %ld\n"), (long)super->s_checkinterval);
    ...
    msgprintf(MSG_DEBUG1, tr("default mount options: [%s]\n"), mntopt);
    ...
    msgprintf(MSG_DEBUG1, tr("the filesystem type determined by the features is [%s]\n"), format_fstype(origextfstype));
...
int extfs_mount(char *partition, char *mntbuf, char *fsbuf, int flags, char *mntinfo)
{
    ...
    msgprintf(MSG_DEBUG1, tr("extfs_mount(partition=[%s], mnt=[%s], fsbuf=[%s])\n"), partition, mntbuf, fsbuf);
    ...
    {   msgprintf(MSG_DEBUG1, tr("ext2fs_open(%s) failed\n"), partition);
        return -1;
    }
    ...
    msgprintf(MSG_VERB2, tr("the filesystem of [%s] type determined by the features is [%s]\n"), partition, fsname);
    
    ext2fs_close(fs);
    
    if (strcmp(fsname, fsbuf)!=0)
    {   msgprintf(MSG_DEBUG1, tr("extfs_mount: the filesystem requested [%s] does not match the filesystem detected [%s]\n"), fsbuf, fsname);
        return -1;
    }
...
int extfs_test(char *partition, int extfstype) // returns true if it's that sort of filesystem
{
    ...
    extfstypedetected=extfs_get_fstype_from_compat_flags((u64)fs->super->s_feature_compat, 
            (u64)fs->super->s_feature_incompat, (u64)fs->super->s_feature_ro_compat);
    msgprintf(MSG_DEBUG1, tr("the filesystem type determined by the extfs features is [%s]\n"), format_fstype(extfstypedetected));
...
u64 check_prog_version(char *prog)
{
    ...
    {   errprintf(tr("program %s was not found or has bad permissions.\n"), prog);
        return -1;
    }
    ...
    {   errprintf(tr("can't parse %s version number: no match\n"), prog);
        return 0;
    }
    ...
    if (x==0 && y==0)
    {   errprintf(tr("can't parse %s version number: x=y=0\n"), prog);
        return 0;
    }
