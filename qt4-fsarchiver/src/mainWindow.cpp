        ...
        if (geteuid() == 0) // 0 is the UID of the root  1000 von Dieter
	{
           ...
           while (j > 1)
	   {
		...
		partition_ = "/dev/"+ partition_; 
                ...
                part_art = mtab_einlesen(partition_);
...
QString MWindow::mtab_einlesen(QString partition_if_home)
{
   ...
   if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
     text = ds.readLine();
   	while (!ds.atEnd())
      	{
 	      if (text.indexOf(partition_if_home) > -1)  // Partitionsname gefunden
      	    { 
...
QString befehl;
   ...
   /* befehl = "kill(getpid(), SIGUSR1)";  //fsarchiver abbrechen
    // system (befehl.toAscii().data());
   //  befehl = "kill(getppid(), SIGUSR1)";  //fsarchiver abbrechen
   //  system (befehl.toAscii().data());
